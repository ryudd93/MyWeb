...
public class PageVO {
	...
	public PageVO(int pageNum, int amount, int total) {
		...
		//1. endPage계산
		// 현재 페이지 번호 1~10이다 -> 보여져야할 끝 번호 10
		// 현재 페이지 번호  14이다 -> 보여져야할 끝 번호 20
		// 공식 : (int)Math.ceil(페이지번호 / 화면에 보여질 페이지 수) * 화면에 보여질 페이지 수
		
		this.endPage = (int)Math.ceil(this.pageNum / 10.0) * 10;
		
		//2. startPage계산
		// 공식 : 끝 페이지 - 화면에 보여질 페이지 수 + 1
		this.startPage = this.endPage - 10 + 1;
		
		//3. realEnd 실제 끝 번호
		// 만약에 게시글이 52개라면? -> 페이지 끝 번호 6
		// 만약에 게시글이 105개라면? -> 페이지 끝 번호 11
		// 공식 : (int)Math.ceil(전체 게시글 후 / 페이지에 보여지는 데이터 개수)
		int realEnd = (int)Math.ceil(this.total / (double)this.amount);
		
		//마지막 페이지 도달시에 보여져야하는 끝번호가 달라짐
		//ex : 131개 게시물
		//1번 페이지 클릭시 -> endPage = 10, realEnd = 14 (endPage로 세팅)
		//11번 페이지 클릭시 -> endPage = 20, realEnd = 14 (realPage로 세팅)
		//결론 endPage > realEnd라면 realEnd를 보여주면 됨
		if (endPage > realEnd) {
		...
		//확인
		System.out.println("시작페이지 : " + this.startPage + ", 끝 페이지 " + this.endPage);
